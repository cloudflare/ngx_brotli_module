...
extern "C" {
  ...
  void *newBrotli(int quality) {
    return new BrotliCompressor(BrotliParams(quality));
  }

  void freeBrotli(void *compressor) {
    BrotliCompressor *c = static_cast<BrotliCompressor*>(compressor);
        delete(c);
    }
  ...
  brotliRet brotli_compress(b_streamp state, brotliFlush f) {
    ...
    while (1) {
      if (state->avail_out == 0 && state->avail_in == 0) {
        return B_BUF_ERROR;
      }
      if (state->buffered_out) {
        if (state->avail_out) {
          if (state->avail_out >= state->buffered_out) {
            memcpy(state->next_out, state->out_buff, state->buffered_out);

            state->total_out += state->buffered_out;

            state->next_out += state->buffered_out;
            state->avail_out -= state->buffered_out;

            state->out_buff += state->buffered_out;
            ...
            memcpy(state->next_out, state->out_buff, state->avail_out);

            state->total_out += state->avail_out;

            state->next_out += state->avail_out;

            state->out_buff += state->avail_out;
            state->buffered_out -= state->avail_out;
      ...
      if (state->avail_in) {
        ...
        bool flush = (f == B_SYNC_FLUSH);
        bool last = (f == B_FINISH);

        BrotliMemIn in(state->next_in, state->avail_in);
        unsigned long bytesCopied = CopyOneBlockToRingBuffer(&in, c);

        state->avail_in -= bytesCopied;
        state->next_in += bytesCopied;
        state->total_in += bytesCopied;

        last &= (state->avail_in == 0);
        c->WriteBrotliData(last, flush, &state->buffered_out, &state->out_buff);
        if (last) {
          state->finish = 1;
        }
      } else if (state->finish == 0 && f == B_FINISH) {
        c->WriteBrotliData(true, true, &state->buffered_out, &state->out_buff);
        state->finish = 1;
      } else {
        return B_OK;
      }
